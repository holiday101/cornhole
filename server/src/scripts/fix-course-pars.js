...
// courses with corrected, sourced values (see conversation/commit history
// for where the numbers came from). Safe to re-run - it always rewrites
// holes_count and course_holes for each named course from scratch.
//
...
const updateCourse = db.prepare('UPDATE courses SET holes_count = ? WHERE id = ?');
...
db.transaction(() => {
  for (const { name, pars } of FIXES) {
    ...
    updateCourse.run(pars.length, course.id);
    deleteHoles.run(course.id);
    pars.forEach((par, i) => insertHole.run(course.id, i + 1, par));
    results.push(`Fixed "${name}" (id ${course.id}): ${pars.length} holes, par ${pars.reduce((a, b) => a + b, 0)}`);
  }
})();
